...
function Delstatic(param1, mcoord)
  env.info("DEBUG: Delstatic called....")
  ...
  if param1 == nil or tonumber(param1)
    ...
    then if param1 == nil
         ...
         --deleteZone:Scan({Object.Category.CARGO,Object.Category.STATIC,})
         deleteZone:Scan({Object.Category})
         local cargoset = SET_STATIC:New():FilterZones({deleteZone}):FilterOnce()
         cargoset:ForEachStatic(function (u)
          u:Destroy()
         
          local deleteTable = deleteZone:GetScannedUnits()
          
          for i=1,#deleteTable do
            env.info("DEBUG: delete table entry")
          end
...
function GMFunc.FFE(mcoord,GrpName,sdistance)

--local grpname = GrpName

  west = mcoord:Translate(sdistance, 270, false, false)
  east = mcoord:Translate(sdistance, 90, false, false)
  north = mcoord:Translate(sdistance, 0, false, false)
  south = mcoord:Translate(sdistance, 180, false, false)
  northeast = mcoord:Translate(sdistance, 45, false, false)
  southwest = mcoord:Translate(sdistance, 225, false, false)
  northwest = mcoord:Translate(sdistance, 315, false, false)
  southeast = mcoord:Translate(sdistance, 135, false, false)
  
  --local grpname = GROUP:FindByName(GrpName)
  env.info("GM DEBUG: " .. GrpName)
  
    if GrpName:find("SHELLDRAKE") then       
      groupName = ARTY_SHELLDRAKE 
       shots=5
    elseif GrpName:find("PUNISHER") then    
     groupName = ARTY_PUNISHER 
      shots=5    
    elseif GrpName:find("VULCAN") then    
     groupName = ARTY_VULCAN
      shots=5
    elseif GrpName:find("IOWA") then    
     groupName = ARTY_IOWA
      shots=6
      weapontype= ARTY.WeaponType.Cannon  
    elseif GrpName:find("MURPHY") then    
     groupName = SHIP_MURPHY
      shots=1   
     
    end
    
  groupName:AssignTargetCoord(mcoord,  1, 1,  shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(west,  1, 1,   shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(east,  1, 1,   shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(north,  1, 1,   shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(south,  1, 1,   shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(northeast,  1, 1,   shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(southwest,  1, 1,   shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(northwest,  1, 1,  shots,nil,nil,weapontype)
  groupName:AssignTargetCoord(southeast,  1, 1,   shots,nil,nil,weapontype)

  MESSAGE:New("Fire Control: Target package for effect received..." ,12,""):ToAll()
  
  
end

function EnemyCamp(mcoord)
  env.info("DEBUG: EnemyCampStart...")
  Vec2 = mcoord:GetVec2()
  campZone = ZONE_RADIUS:New("campzone", mcoord:GetVec2(), 50)

  campZones = {campZone}
  campset = SET_GROUP:New():FilterPrefixes("campheavy-"):FilterOnce()
  camptemps = {}

  campset:ForEach(function(z)
     local gname = z:GetName() 
     table.insert(camptemps,gname)
  end)
  _args = {campZones,camptemps}
  SpawnCamps(_args)
  --env.info("DEBUG: EnemyCampEnd...")

end

function dronesupport(param1,mcoord)

  local drone = SPAWN:New("drone"):OnSpawnGroup(function (grp)
    
    if param1 and param1 == "lase" then 
    
      TIMER:New(JTAC):Start(15)      
    
    end
    local distance = grp:GetCoordinate():Get2DDistance(mcoord) / 1000
    local eta1 = distance / 400
    local eta2 = eta1 * 60 
    
    MESSAGE:New("ISR Enroute. Distance: " ..  math.ceil(distance) .. "km.  Eta: " .. math.ceil(eta2) .. " mikes." ,45,""):ToAll()
    
    local orbit_task = grp:TaskOrbitCircle(4000, 200, mcoord)

    grp:SetTask(orbit_task)
...
function SpawnStaticInf(mcoord)
  local Heading = math.random(1,359)
  staticinf = SPAWNSTATIC:NewFromStatic( "staticinf", country.id.SYRIA )
  local infguy = staticinf:SpawnFromCoordinate( mcoord, Heading + 3, "ZeusInf-" .. math.random(1,3000)  )
...
function ListStatics (param1)
  staticSet = SET_STATIC:New():FilterOnce()
  staticSet:ForEachStatic(function (static)
    if param1 and string.match(static:GetName(), param1) then
      MESSAGE:New("" .. static:GetName() ,35,""):ToAll()
    end
  end)
end

function ListGroups(param1)
  if param1 == nil then
    return
  end
  staticSet = SET_GROUP:New():FilterOnce()
  staticSet:ForEachGroup(function (grp)
    if string.match(grp:GetName(), "#")  then
    --next
    elseif param1 and string.match(grp:GetName(), param1) then
      MESSAGE:New("" .. grp:GetName() ,35,""):ToAll()
    end
  end)
end

function createCSAR(mcoord)

local _zone = ZONE_RADIUS:New("csarZone",mcoord:GetVec2(),5)
BlueCsar:SpawnCSARAtZone( _zone, coalition.side.BLUE, "Johnny Mac", false )

end

function addtroops (param1,mcoord)

  -- Vec2zone = mcoord:GetVec2()
  if param1 == nil then
    env.info("DEBUG: param 1 empty!")
    return
  end
  if mcoord == nil then  
    env.info("DEBUG: mcoord is nil! ")
  else
     Vec2zone = mcoord:GetVec2()
     env.info("DEBUG: mcoord exists! ")
         
  end
  env.info("DEBUG: param 1! " .. param1)
  
  local groupname = param1
  local ctldgroup = GROUP:FindByName(groupname)
  local unit_table = ctldgroup:GetUnits()
  local unitcount = #unit_table
  env.info("unitcount:" .. unitcount)
  local Zone_namestring = "Zone_" .. groupname
  local Zone_ = ZONE_RADIUS:New(Zone_namestring,Vec2zone,5)

  local InjectTroops = CTLD_CARGO:New(nil,groupname,{groupname},CTLD_CARGO.Enum.TROOPS,true,true,unitcount,nil,false,100)
...
function IC()
  ...
  if param1 == nil or param2 == nil then
    env.info("DEBUG: param 1 or 2 empty!")
    return
  end
  if mcoord == nil then  
    env.info("DEBUG: mcoord is nil! ")
  ...
  env.info("DEBUG: param 1! " .. param1)
  env.info("DEBUG: param 2! " .. param2)
  
  local cargoname = param1
  local cargomass = param2

  local Zone_ = ZONE_RADIUS:New("tempzone",Vec2zone,5)

  bsdctldInjectCrate("tempzone", cargoname, cargomass)
...
function GMFunc.ShowCoord(mcoord,param1)
  ...
  local LatDecimal = LatDeg + LatMin/60 + LatSec/3600
  local LongDecimal = LatDeg + LatMin/60 + LatSec/3600
  ...
  local LongDecimal = LongDeg + LongMin/60 + LongSec/3600
  env.info("DEBUG: ns430" .. LatDecimal)
  
  local ns430string = "FIX;" .. string.sub(LongDecimal,1,6) .. ";" .. string.sub(LatDecimal,1,6) .. ";NAME"
  ...
  if param1 == "430" or param1 == "ns430" then
     CoordMarker = MARKER:New(mcoord, "\nMGRS: " .. MGRSString .. "\nNS430: " .. ns430string)  
  else
     CoordMarker = MARKER:New(mcoord, "LL: " .. LLString .. "\nLL Dec: " .. DecString .. "\nLL Prec: " .. PrecString .. "\nMGRS: " .. MGRSString)
  end
...
function GMFunc.MarkHandler:onEvent(event)
    ...
    elseif event.id == 27 and string.find(event.text, GMFunc.CmdSymbol) and (GMFunc.PW == nil or string.find(event.text, GMFunc.PW) ~= nil)
      ...
      then if DebugMode == true
        ...
        ListStatics(allParams.param1)
        
        

      elseif string.find(cmd, "lg")
      ...
      then if DebugMode == true
        ...
        ListGroups(allParams.param1)

      elseif string.find(cmd, "CSAR")
      ...
      elseif string.find(cmd, "addtroops")
      ...
      then if DebugMode == true
        ...
        addtroops(allParams.param1,mcoord)

      elseif string.find(cmd, "IC")
      ...
      then if DebugMode == true
        ...
        IC(allParams.param1,allParams.param2,mcoord) 
               
      elseif string.find(cmd, "dst")
      ...
      then if DebugMode == true
        ...
        env.info("DEBUG: Delstatic requested....")
        
        Delstatic(allParams.param1,mcoord)

      elseif string.find(cmd, "CAMP")
      ...
      elseif string.find(cmd, "dronesupport")
      ...
      then if DebugMode == true
        ...
        dronesupport(allParams.param1,mcoord)

      elseif string.find(cmd, "si")
      ...
      then if GMFunc.DebugMode == true
        ...
        GMFunc.FFE( mcoord,allParams.param1,allParams.param2,allParams.param3)
         --Fargo
                     ...
                     elseif cmd == "coord" 
                      ...
                      then if GMFunc.DebugMode == true
                           ...
                           GMFunc.ShowCoord(mcoord,allParams.param1)
