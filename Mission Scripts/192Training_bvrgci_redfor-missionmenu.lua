env.info("bvr_redfor loading", false)
...
--BVRGCI.rangeRadio = (JTF1.rangeRadio and JTF1.rangeRadio or BVRGCI.defaultRadio)
  ...
  BVRGCI.ZoneBvr = ZONE:FindByName("ZONE_BVR_REDFOR")
  --- ME Zone object for adversary spawn point
  -- @field #string ZoneBvrSpawn 
  BVRGCI.ZoneBvrSpawn = ZONE:FindByName("ZONE_BVR_SPAWN_REDFOR")
  --- ME Zone object for adversary spawn waypoint 1
  -- @field #string ZoneBvrWp1 
  BVRGCI.ZoneBvrWp1 = ZONE:FindByName("ZONE_BVR_WP1_REDFOR")
  ...
  function BVRGCI.SpawnType(typeName, typeSpawnTemplate, Qty, Altitude, Formation) 
            ...
            function (CheckAdversary)
              if SpawnGroup then
                ...
                if (SpawnGroup:IsNotInZone(BVRGCI.ZoneBvr) or (BVRGCI.Destroy)) then 
                  ...
                  local msgDestroy = "All players, BVR adversary group " .. groupName .. " removed from BVR Redfor Range."
                  local msgLeftZone = "All players, BVR adversary group " .. groupName .. " left zone and was removed from BVR Redfor Range."
                  SpawnGroup:Destroy()
                  SpawnGroup = nil
    ...
    local msg = "All players, BVR Adversary group spawned in BVR Redfor Range."
    if MISSIONSRS.Radio then -- if MISSIONSRS radio object has been created, send message via default broadcast.
      MISSIONSRS:SendRadio(msg, BVRGCI.rangeRadio)
    else -- otherwise, send in-game text message
      MESSAGE:New(msg):ToAll()
  ...
  function BVRGCI.BuildMenuType(ParentMenu)
    for i, v in ipairs(BVRGCI.Adversary) do
      ...
      if GROUP:FindByName(typeSpawnTemplate) ~= nil then
          MENU_MISSION_COMMAND:New(typeName, ParentMenu, BVRGCI.SpawnType, typeName, typeSpawnTemplate, BVRGCI.Spawn.Qty, BVRGCI.Spawn.Level, ENUMS.Formation.FixedWing[BVRGCI.Spawn.Formation][BVRGCI.Spawn.Spacing])
      else
  ...
  function BVRGCI.BuildMenuSpacing(Spacing, ParentMenu)
    ...
    BVRGCI.SubMenu[MenuName] = MENU_MISSION:New(MenuText, ParentMenu)
  ...
  function BVRGCI.BuildMenuRoot()
    ...
    BVRGCI.Menu = MENU_MISSION:New("AI BVR/GCI", REDFOR)
      ...
      -- level 2 command
      BVRGCI.MenuRemoveAdversaries = MENU_MISSION_COMMAND:New("Remove BVR Adversaries in BVR Redfor Range", BVRGCI.Menu, BVRGCI.RemoveAdversaries)
  end
  
  BVRGCI.BuildMenuRoot()
  
--- END BVRGCI SECTION
env.info("bvr_redfor complete", false)
