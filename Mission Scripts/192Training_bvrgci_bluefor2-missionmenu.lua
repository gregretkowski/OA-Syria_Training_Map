env.info("bvr_bluefor loading", false)
...
--BVRGCI.rangeRadio = (JTF1.rangeRadio and JTF1.rangeRadio or BVRGCI.defaultRadio)
  ...
  BVRGCI.Adversary = { 
    ...
    {"Transport", "BVR_BLUEFOR_TRANSPORT2"},
    {"MiG-21", "BVR_BLUEFOR_MiG-212"},
  ...
  function BVRGCI.SpawnType(typeName, typeSpawnTemplate, Qty, Altitude, Formation) 
            ...
            function (CheckAdversary)
              if SpawnGroup then
                ...
                if (SpawnGroup:IsNotInZone(BVRGCI.ZoneBvr) or (BVRGCI.Destroy)) then 
                  ...
                  local msgDestroy = "All players, BVR adversary group " .. groupName .. " removed from BVR Range #2."
                  local msgLeftZone = "All players, BVR adversary group " .. groupName .. " left zone and was removed from BVR Range #2."
                  SpawnGroup:Destroy()
                  SpawnGroup = nil
    ...
    local msg = "All players, BVR Adversary group spawned in BVR Range #2."
    if MISSIONSRS.Radio then -- if MISSIONSRS radio object has been created, send message via default broadcast.
      MISSIONSRS:SendRadio(msg, BVRGCI.rangeRadio)
    else -- otherwise, send in-game text message
      MESSAGE:New(msg):ToAll()
  ...
  function BVRGCI.BuildMenuRoot()
    --BLUEFOR = MENU_MISSION:New("BLUEFOR")
    BVRGCI.Menu = MENU_MISSION:New("AI BVR/GCI #2", A2Aroot)
      ...
      -- level 2 command
      BVRGCI.MenuRemoveAdversaries = MENU_MISSION_COMMAND:New("Remove BVR Adversaries in BVR Range #2", BVRGCI.Menu, BVRGCI.RemoveAdversaries)
  end
  
  BVRGCI.BuildMenuRoot()
  
--- END BVRGCI SECTION
env.info("bvr_bluefor complete", false)
