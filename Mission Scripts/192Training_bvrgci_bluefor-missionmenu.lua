...
--BVRGCI.rangeRadio = (JTF1.rangeRadio and JTF1.rangeRadio or BVRGCI.defaultRadio)
  ...
  function BVRGCI.SpawnType(typeName, typeSpawnTemplate, Qty, Altitude, Formation) 
            ...
            function (CheckAdversary)
              if SpawnGroup then
                ...
                if (SpawnGroup:IsNotInZone(BVRGCI.ZoneBvr) or (BVRGCI.Destroy)) then 
                  ...
                  local msgDestroy = "All players, BVR adversary group " .. groupName .. " removed from BVR Range #1."
                  local msgLeftZone = "All players, BVR adversary group " .. groupName .. " left zone and was removed from BVR Range #1."
                  SpawnGroup:Destroy()
                  SpawnGroup = nil
    ...
    local msg = "All players, BVR Adversary group spawned in BVR Range #1."
    if MISSIONSRS.Radio then -- if MISSIONSRS radio object has been created, send message via default broadcast.
      MISSIONSRS:SendRadio(msg, BVRGCI.rangeRadio)
    else -- otherwise, send in-game text message
      MESSAGE:New(msg):ToAll()
  ...
  function BVRGCI.BuildMenuRoot()
    ...
    BVRGCI.Menu = MENU_MISSION:New("AI BVR/GCI #1", A2Aroot)
      ...
      -- level 2 command
      BVRGCI.MenuRemoveAdversaries = MENU_MISSION_COMMAND:New("Remove BVR Adversaries in BVR Range #1", BVRGCI.Menu, BVRGCI.RemoveAdversaries)
  end
