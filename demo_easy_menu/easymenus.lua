...
local function DefineMenusFromHierarchy(hierarchy, grp)
  --MENU_GROUP:New(grp, "Display", parentMnu)
  --MENU_GROUP_COMMAND:New(grp, "Display", parentMnu, callback, args)
  ...
  for i = 1, #keys, 1 do
    ...
    elseif (entry['flag'] == nil) then
      ...
      entry['mnu'] = MENU_GROUP:New(grp, entry['disp'], parentEntry['mnu'])
      --entry['mnu'] = '<key='..keys[i]..'>'
      --print('MENU_GROUP:New(grp, '..entry['disp']..', '..(parentEntry['mnu'] or 'nil')..')')
    else
      local parentEntry = hierarchy[entry['parentPath']]
      entry['mnu'] = MENU_GROUP_COMMAND:New(grp, entry['disp'], parentEntry['mnu'], SetFlag, entry['flag'])
      --entry['mnu'] = '<key='..keys[i]..'>'
...
BASE:HandleEvent(EVENTS.PlayerEnterAircraft)

function BASE:OnEventPlayerEnterAircraft(EventData)
   env.info("PlayerEnteredAircraft=" .. EventData.IniPlayerName)
   env.info("UnitType=" .. EventData.IniTypeName)

   local grp = GROUP:FindByName(EventData.IniGroupName)
   local hierarchy = BuildMenuHierarchy(easyMenu)  
   DefineMenusFromHierarchy(hierarchy, grp)
end
