...
env.info("initializing My LotAtc Link")
local LCallbacks = {}
--------------------------------------------------------------
...
loe_order_position = function( env, order, _group )
    ...
    local FromCoord = _group:GetCoordinate()
    env.info( FromCoord )
    local _pos = COORDINATE:NewFromLLDD(order.value.latitude, order.value.longitude)--, FromCoord:GetPointVec2():GetAlt())
...
env.info("LotAtc Link initialized")